...
def main():
    ...
    # optimizer = tf.train.AdamOptimizer(learning_rate, beta1=0.9, beta2=0.999, epsilon=0.0001).minimize(loss, global_step=global_step)
    ...
    # Create a summary to monitor learning_rate, loss, accuracy  # ADD
    tf.summary.scalar('learning_rate', learning_rate)
    tf.summary.scalar("loss", loss)
    tf.summary.scalar("accuracy", accuracy)
    # Merge all summaries into a single op       # ADD
    ...
    # Create the restore which will be used to restore 'variables_to_restore'.
    ...
    with tf.Session() as sess:
        ...
        # op to write logs to Tensorboard   # ADD
        ...
        while step < training_iters:
            ...
            summary = sess.run(merged_summary_op, feed_dict={x: batch_xs, y: batch_ys, keep_var: keep_rate})   # ADD
            # Write logs at every iteration
            train_writer.add_summary(summary, step)                                                     # ADD
            ...
            if step % display_step == 0:
                ...
                # print('Learning rate: %f' % (sess.run(optimizer._lr, feed_dict={x: batch_xs, y: batch_ys, keep_var: 1.})))
